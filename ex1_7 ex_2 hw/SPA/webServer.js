...
var UITmodels = require("./modelData/photoApp.js").UITmodels;
...
app.get("/user/list", function (request, response) {
  response.status(200).send(UITmodels.userListModel());
  return;
});
...
app.get("/user/:id", function (request, response) {
  ...
  var user = UITmodels.userModel(id);
  if (user === null) {
...
app.get("/photosOfUser/:id", function (request, response) {
  var id = request.params.id;
  var photos = UITmodels.photoOfUserModel(id);
  if (photos.length === 0) {
